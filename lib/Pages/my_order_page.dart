...
class MyOrderPageState extends State<MyOrderPage> {
  ...
  loadMoreData() async {
    ...
    API.getOrderPageList(start).then((value) {
      setState(() {
  ...
  Widget createItem(List<OrderModel> list, int index) {
    OrderModel myOrderModel = list[index];
    return Column(
      children: <Widget>[
        InkWell(
          onTap: () {
  ...
  Widget refundWidget(int index, OrderModel orderModel) {
    if (index == 0) {
      if (orderModel.order.orderStatus == 1) {//代发货
        if (orderModel.order.refundStatus == 0) {//发起退款
        return RefundStatusWidget(string: "退款中",color:Color.fromARGB(255, 250, 104, 14) ,function: (){
          Navigator.push(context,
              new MaterialPageRoute(builder: (context) {
          ...
          return RefundStatusWidget(string: "退款中",color:ApplicationUtils.orangeColor ,function: (){
            Navigator.push(context,
                new MaterialPageRoute(builder: (context) {
                  return RefuseRefundPage(orderModel: orderModel,);
                }));
          },);
          ...
          return RefundStatusWidget(string: "退款",color:ApplicationUtils.grey ,function: (){
            Navigator.push(context,
                new MaterialPageRoute(builder: (context) {
